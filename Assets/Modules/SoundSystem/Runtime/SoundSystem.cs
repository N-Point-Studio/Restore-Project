...
namespace Modules.SoundSystems
{
    ...
    {
        [SerializeField] protected SoundDatabase database;
        ...
        /// When set to true, new UI sound audios that have the same audio clip as any other UI sound audios, will be ignored
        /// </summary>
        public bool IgnoreDuplicateAmbienceSounds { get; set; }

        /// <summary>
        /// When set to true, new UI sound audios that have the same audio clip as any other UI sound audios, will be ignored
        /// </summary>
        public bool IgnoreDuplicateVoiceSounds { get; set; }
        ...
        /// </summary>
        public float GlobalAmbienceVolume { get; set; }

        /// <summary>
        /// Global Voice volume
        /// </summary>
        public float GlobalVoiceVolume { get; set; }
        ...
        private Dictionary<int, Audio> ambienceAudio;
        private Dictionary<int, Audio> voiceAudio;
        ...
        protected void Awake()
        {
            ...
            ambienceAudio = new Dictionary<int, Audio>();
        ...
        private void OnSceneLoaded(Scene scene, LoadSceneMode mode)
        {
            ...
            RemoveNonPersistAudio(ambienceAudio);
        ...
        private bool GetAudioTypeIgnoreDuplicateSetting(Audio.AudioType audioType)
        {
            switch (audioType)
            {
                ...
                case Audio.AudioType.Ambience:
                    return IgnoreDuplicateAmbienceSounds;
                case Audio.AudioType.Voice:
        ...
        public Audio GetAudio(AudioClip audioClip)
        {
            ...
            audio = GetAmbienceAudio(audioClip);
            if (audio != null)
            {
                return audio;
            }

            audio = GetVoiceAudio(audioClip);
            if (audio != null)
            {
                return audio;
            }
        ...
        /// <param name="audioID">The id of the UI sound fx Audio to be returned</param>
        /// <returns>UI sound fx Audio that has as its id the audioID if one is found, null if no such Audio is found</returns>
        public Audio GetAmbienceAudio(int audioID)
        {
            return GetAudio(Audio.AudioType.Ambience, true, audioID);
        }
        ...
        /// <param name="audioClip">The audio clip of the UI sound Audio to be retrieved</param>
        /// <returns>First occurrence of UI sound Audio that has as plays the audioClip, null if no such Audio is found</returns>
        public Audio GetAmbienceAudio(AudioClip audioClip)
        {
            return GetAudio(Audio.AudioType.Ambience, true, audioClip);
        }
        ...
        /// <param name="audioID">The id of the UI sound fx Audio to be returned</param>
        /// <returns>UI sound fx Audio that has as its id the audioID if one is found, null if no such Audio is found</returns>
        public Audio GetVoiceAudio(int audioID)
        {
        ...
        /// <param name="audioClip">The audio clip of the UI sound Audio to be retrieved</param>
        /// <returns>First occurrence of UI sound Audio that has as plays the audioClip, null if no such Audio is found</returns>
        public Audio GetVoiceAudio(AudioClip audioClip)
        {
        ...
        /// <param name="clip">The audio clip to prepare</param>
        /// <returns>The ID of the created Audio object</returns>
        public int PrepareAmbience(AudioClip clip)
        {
            return PrepareAudio(Audio.AudioType.Ambience, clip, 1f, false, false, 0f, 0f, -1f, null);
        }
        ...
        /// <param name="volume"> The volume the music will have</param>
        /// <returns>The ID of the created Audio object</returns>
        public int PrepareAmbience(AudioClip clip, float volume)
        {
            return PrepareAudio(Audio.AudioType.Ambience, clip, volume, false, false, 0f, 0f, -1f, null);
        }
        ...
        /// <param name="loop">Wether the sound is looped</param>
        /// <returns>The ID of the created Audio object</returns>
        public int PrepareAmbience(AudioClip clip, bool loop)
        {
            return PrepareAudio(Audio.AudioType.Ambience, clip, 1f, loop, false, 0f, 0f, -1f, null);
        }
        ...
        /// <param name="sourceTransform">The transform that is the source of the sound (will become 3D audio). If 3D audio is not wanted, use null</param>
        /// <returns>The ID of the created Audio object</returns>
        public int PrepareAmbience(AudioClip clip, float volume, bool loop, Transform sourceTransform)
        {
            return PrepareAudio(Audio.AudioType.Ambience, clip, volume, loop, false, 0f, 0f, -1f, sourceTransform);
        }
        ...
        /// <param name="clip">The audio clip to prepare</param>
        /// <returns>The ID of the created Audio object</returns>
        public int PrepareVoice(AudioClip clip)
        {
        ...
        /// <param name="volume"> The volume the music will have</param>
        /// <returns>The ID of the created Audio object</returns>
        public int PrepareVoice(AudioClip clip, float volume)
        {
        ...
        /// <param name="loop">Wether the sound is looped</param>
        /// <returns>The ID of the created Audio object</returns>
        public int PrepareVoice(AudioClip clip, bool loop)
        {
            return PrepareAudio(Audio.AudioType.Voice, clip, 1f, loop, false, 0f, 0f, -1f, null);
        }
        ...
        /// <param name="sourceTransform">The transform that is the source of the sound (will become 3D audio). If 3D audio is not wanted, use null</param>
        /// <returns>The ID of the created Audio object</returns>
        public int PrepareVoice(AudioClip clip, float volume, bool loop, Transform sourceTransform)
        {
            return PrepareAudio(Audio.AudioType.Voice, clip, volume, loop, false, 0f, 0f, -1f, sourceTransform);
        }

        public int PrepareAudio(string clipId)
        {
            SoundDatabase.ItemPair item = database.Get(clipId);
            return PrepareAudio(item.Type, item.Value, 1f, false, false, 0f, 0f, -1f, null);
        }

        public int PrepareAudio(Audio.AudioType audioType, string clipId, float volume, bool loop, bool persist, float fadeInSeconds, float fadeOutSeconds, float currentMusicfadeOutSeconds, Transform sourceTransform)
        {
            SoundDatabase.ItemPair item = database.Get(clipId);
            return PrepareAudio(audioType, item.Value, volume, loop, persist, fadeInSeconds, fadeOutSeconds, currentMusicfadeOutSeconds, sourceTransform);
        }

        private int PrepareAudio(Audio.AudioType audioType, AudioClip clip, float volume, bool loop, bool persist, float fadeInSeconds, float fadeOutSeconds, float currentMusicfadeOutSeconds, Transform sourceTransform, float pitch = 1)
        {
            if (clip == null)
            {
            ...
            Audio audio = new Audio(this, audioType, clip, loop, persist, volume, fadeInSeconds, fadeOutSeconds, sourceTransform);
            audio.Pitch = pitch;
        ...
        public int PlayAudio(string clipId, float volume = 1, bool loop = false, bool ignoreDuplicate = false, bool persist = false, Transform sourceTransform = null, float fadeInSeconds = 0f, float fadeOutSeconds = 0f)
        {
            SoundDatabase.ItemPair item = database.Get(clipId);

            if (ignoreDuplicate)
            {
                Dictionary<int, Audio> audioDict = GetAudioTypeDictionary(item.Type);

                List<int> keys = new List<int>(audioDict.Keys);
                foreach (int key in keys)
                {
                    Audio audio = audioDict[key];
                    if (audio.Clip == item.Value)
                    {
                        return key;
                    }
                }
        ...
        {
            bool isSoundPlaying = GetAudio(clip) != null;

            float pitch = 1;
            if (database.RandomizePitchForDuplicates)
            {
                pitch = Random.Range(database.MinPitch, database.MaxPitch);
            }

            return PlayAudio(Audio.AudioType.Sound, clip, 1f, false, false, 0f, 0f, -1f, null, pitch);
        }
        ...
        public int PlaySound(AudioClip clip, float volume, bool loop, float pitch)
        {
            return PlayAudio(Audio.AudioType.Sound, clip, volume, loop, false, 0f, 0f, -1f, null, pitch);
        }
        ...
        /// <param name="clip">The audio clip to play</param>
        /// <returns>The ID of the created Audio object</returns>
        public int PlayAmbience(AudioClip clip)
        {
            return PlayAudio(Audio.AudioType.Ambience, clip, 1f, false, false, 0f, 0f, -1f, null);
        }
        ...
        /// <param name="volume"> The volume the music will have</param>
        /// <returns>The ID of the created Audio object</returns>
        public int PlayAmbience(AudioClip clip, float volume)
        {
            return PlayAudio(Audio.AudioType.Ambience, clip, volume, false, false, 0f, 0f, -1f, null);
        }
        ...
        /// <param name="loop">Wether the sound is looped</param>
        /// <returns>The ID of the created Audio object</returns>
        public int PlayAmbience(AudioClip clip, bool loop)
        {
            return PlayAudio(Audio.AudioType.Ambience, clip, 1f, loop, false, 0f, 0f, -1f, null);
        }
        ...
        /// <param name="sourceTransform">The transform that is the source of the sound (will become 3D audio). If 3D audio is not wanted, use null</param>
        /// <returns>The ID of the created Audio object</returns>
        public int PlayAmbience(AudioClip clip, float volume, bool loop, Transform sourceTransform)
        {
            return PlayAudio(Audio.AudioType.Ambience, clip, volume, loop, false, 0f, 0f, -1f, sourceTransform);
        }
        ...
        /// <param name="volume"> The volume the music will have</param>
        /// <returns>The ID of the created Audio object</returns>
        public int PlayVoice(AudioClip clip, float volume)
        {
        ...
        /// <param name="loop">Wether the sound is looped</param>
        /// <returns>The ID of the created Audio object</returns>
        public int PlayVoice(AudioClip clip, bool loop)
        {
            return PlayAudio(Audio.AudioType.Voice, clip, 1f, loop, false, 0f, 0f, -1f, null);
        }
        ...
        /// <param name="sourceTransform">The transform that is the source of the sound (will become 3D audio). If 3D audio is not wanted, use null</param>
        /// <returns>The ID of the created Audio object</returns>
        public int PlayVoice(AudioClip clip, float volume, bool loop, Transform sourceTransform)
        {
            return PlayAudio(Audio.AudioType.Voice, clip, volume, loop, false, 0f, 0f, -1f, sourceTransform);
        }

        private int PlayAudio(Audio.AudioType audioType, AudioClip clip, float volume, bool loop, bool persist, float fadeInSeconds, float fadeOutSeconds, float currentMusicfadeOutSeconds, Transform sourceTransform, float pitch = 1)
        {
            int audioID = PrepareAudio(audioType, clip, volume, loop, persist, fadeInSeconds, fadeOutSeconds, currentMusicfadeOutSeconds, sourceTransform, pitch);
        ...
        public void StopAudio(int audioId, float fadeOutSeconds = 1)
        {
        ...
        {
            SoundDatabase.ItemPair item = database.Get(id);
        ...
        {
            StopAllAudio(Audio.AudioType.Ambience, -1f);
        }
        ...
        {
            PauseAllAudio(Audio.AudioType.Ambience);
        }
        ...
        {
            ResumeAllAudio(Audio.AudioType.Ambience);
        }

        /// <summary>
        /// Resume all sound fx playing
        /// </summary>
        public void ResumeAllVoices()
        {
